...
export default async function NutritionPage() {
  const res = await fetch(
    "http://localhost:3000/api/products?category=nutrition",
    {
      cache: "no-store",
    },
  );
  ...
  const product: ProductData = {
    _id: dbProduct ? String(dbProduct._id) : "default-nutrition-id",
    slug: "nutrition",
    name: dbProduct?.name ?? "Reset Your Plate",
    subtitle: "Nutrition Guide",
    tag: "NEW",
    tagColor: "bg-[#CC0000] text-white",
    price: dbProduct?.price ?? 24.99,
    originalPrice: 39.99,
    image:
      dbProduct?.image ?? "/ebooks-cover/reset-your-body-nutriton-guide.png",
    upsellOffer,
    headline:
    ...
    reviews: [
      {
        name: "Emma L.",
        stars: 5,
        text: "Lost 8 lbs in the first month without feeling deprived once. This guide is incredible.",
      },
      {
        name: "Carlos M.",
        stars: 5,
        text: "The shopping lists alone are worth it. No more wasted groceries or guessing.",
      },
      {
        name: "Priya S.",
        stars: 5,
        text: "Simple, clean, effective. Finally a nutrition guide that doesn't make me feel overwhelmed.",
      },
      {
        name: "Tom H.",
        stars: 5,
        text: "Combined this with the workout program and the results have been crazy.",
      },
      {
        name: "Jessica F.",
        stars: 5,
        text: "I've tried every diet. This is the first thing that actually stuck.",
      },
      {
        name: "Ryan B.",
        stars: 5,
        text: "Meal plans are so easy to follow. My whole family is eating better now.",
      },
    ],
    recommended: {
      slug: "workout",
